...
`./minGPT` 是整个语言模型的部分，其核心部分来自 https://github.com/karpathy/minGPT<br>
...
`test_no_memory_sample.ipynb`, `test_no_memory.ipynb` 分别实现不带 memory 逻辑的两种训练方式 <br>
`test_no_dc_sample.ipynb`, `test_no_dc.ipynb` 分别实现不带 dynamic cheatsheet 逻辑的两种训练方式 <br>
...
3. 由于 dynamic_cheatsheet 模块调用了千问的 API， 所以 ```test_sample.ipynb```, ```test.ipynb``` 需要在代码开头配置 API key 为环境变量
